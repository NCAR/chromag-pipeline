...
def generate_quadrant_flats(
    ...
    offset_mapping = {'Top-Right': [0], 'Top-Left': [1], 'Bottom-Left': [2], 'Bottom-Right': 3, 'all': [0,1,2,3], 'none': [4]}
    ...
    for dy, dx in offsets:
        ...
        disk = ((Y - (ny/2 + dy))**2 + (X - (nx/2 + dx))**2 <= solar_radius**2).astype(np.float64)
...
def create_flats(start_datetime: datetime.datetime, exptime: float, numflats: int, kll_flag: bool):
    """ 
    ...
    files = glob.glob(path+'20250813/'+'*fits')
...
def main():
    ...
    numflats_help = "number of flats (int) to generate, if --kll will generate 4 x numflats"
