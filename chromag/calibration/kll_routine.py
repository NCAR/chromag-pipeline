...
def gaincalib(logimages, x=None, y=None, object_param=None, maxiter=10,
              silent=False, c=None, shift_flag=0, mask=None):
    """
    ...
    if shift_flag == 0 or x is None or y is None:
        ...
        for ix in range(nx):
            for jy in range(ny):
                ...
                if mask_sum > 1.0:
                    flat[ix, jy] = np.sum((logimages[ix, jy, :] - c) * mask[ix, jy, :]) / mask_sum
                else:
                    flat[ix, jy] = 0.0
        ...
        ss = (nf // 2) - 1
        ref_masked = (logimages[:, :, ss] - np.median(logimages[:, :, ss]) - flat) * mask[:, :, ss]
        
        for k in range(nf):
            tmp = (logimages[:, :, k] - np.median(logimages[:, :, k]) - flat) * mask[:, :, k]
...
def get_flat(files, l=None, m=None, ndata_integ=1.0, 
